...
namespace DataObjects
{
  ...
  public:
    ...
    /** Get access to shared pointer containing workspace porperties. This function is there to provide common interface of iTableWorkspace
      * Despite it is non-constant method, one should be very carefull using it to change the log values when cloning of a table workspace can occur
        as the changes may depend on the order of PeakWorkspace cloning & changes applyed through this pointer.
      * See PeakWorkspaceTest (test_getSetLogAccess) -- for example of this behaviour. 
    ...
    /**Get constant access to shared pointer containing workspace porperties;
       ...
       Meaningfull only for some multithereaded methods when a thread wants to have its own copy of logs   */
  ...
  private:
    ...
    virtual void find(Mantid::Kernel::V3D /*value*/,size_t& /*row*/,const size_t & /*col*/)
    { throw Mantid::Kernel::Exception::NotImplementedError("PeaksWorkspace::find() not implemented."); }
